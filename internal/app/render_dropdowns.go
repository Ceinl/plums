...
func newDropdownFrame(state *State, cfg *RenderConfig, rows int) (dropdownFrame, bool) {
	...
	frame := dropdownFrame{
		...
		y:        y,
		w:        w,
		bg:       ansiBgColor(overlay.Style.Background, theme.BgSurface),
		fg:       ansiFgColor(overlay.Style.Foreground, theme.Text),
		muted:    ansiFgColor(overlay.Style.Muted, theme.TextMuted),
		accent:   ansiFgColor(overlay.Style.Accent, theme.Accent),
